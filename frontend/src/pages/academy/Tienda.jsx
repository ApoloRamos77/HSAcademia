      ...
      <div className="fade-in">
        ...
        {activeTab === 'inventory' && (
          <div className="card fade-in">
            <div className="card-header flex-col md:flex-row items-start md:items-center gap-4 border-b border-border/50 pb-4 mb-4">
              <div className="flex-1">
                <h3 className="card-title flex items-center gap-2"><Package className="text-primary-400" size={20} /> Productos e Inventario</h3>
                <p className="text-muted text-sm mt-1">Registra artículos (uniformes, balones) con stock y precio.</p>
              </div>
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <input 
                  type="text" 
                  className="form-control text-sm py-2 flex-1 md:w-64" 
                  placeholder="Buscar producto o categoría..." 
                  value={searchProduct}
                  onChange={e => setSearchProduct(e.target.value)}
                />
                <button onClick={() => { setSaleForm(initialSale); setShowSaleModal(true); }} className="btn btn-success flex items-center justify-center gap-2 flex-1 md:flex-none whitespace-nowrap">
                  <ShoppingBag size={16} /> Vender
                </button>
                <button onClick={() => { setProductForm(initialProduct); setEditingProductId(null); setShowProductModal(true); }} className="btn btn-primary flex items-center justify-center gap-2 flex-1 md:flex-none whitespace-nowrap">
